...
void conGets( char *buf, int lim )
{
   ...
   for(i = 0; i < lim - 1 && (ch = (char)usart_hitkey()&0xff) != (-1) && ch != '\n' && ch != '\r'; ++i) {
       buf[i] = ch;
		   {putc(ch, (& __stdout));; fflush((& __stdout));} 
	 }
   if(ch == '\n' || ch == '\r'){
       buf[i] = '\r';
       ++i;     
...
{
   if( argc > 0 ) {
      CmdExec( prompt, 1, menu, argc, argv );
   }   else   {
      CmdLoop( prompt, 1, menu );
   }
}
...
int CmdExec( char *prompt, int level, const CMD_ENTRY menu[], int argc, char *argv[] )
{
   ...
   if( (strcmp(argv[0], "?") == 0) || (strcmp(argv[0], "help") == 0) )   {
       ...
       return( 1 );
   }  else if( (cmdP = CmdFind(menu, argv[0]))== 0 )  {
      printf("%s unknown command: '%s'\r\n", prompt, argv[0]);
   }   else if( cmdP->argc == (-1) ) {
       
      (* (FUNC_VARGS)cmdP->func)( argc, &argv[1] );
   }   else if( cmdP->argc != argc )  {
      printf("%s syntax: '%s %s'\r\n", prompt, (cmdP)->str, (cmdP)->syntax);
   }   else if( !cmdP->func )  {
       
      printf("%s command '%s' function missing\r\n", prompt, (cmdP)->str);
   }  else   {
...
void CmdLoop( char *prompt, int level, const CMD_ENTRY menu[] )
{
   ...
   while ( 1 )   {
      {fputs(p, (& __stdout)); fflush((& __stdout));};
      conGets( cmdLine, sizeof(cmdLine) );
      if ((argc = CmdParse( cmdLine, argv )) > 0)      {
         if ((CmdExec( prompt, level, menu, argc, argv )) == 1)  {
           return;
         }
      }
   }
...
void DisplayMenu( char *prompt, int level, const CMD_ENTRY menu[] )
{
   ...
   putc('\r', (& __stdout));;
   putc('\n', (& __stdout));;
   len = printf( "%s Console Commands\r\n", prompt ) - 1;
   while ( len-- )   {
      putc('-', (& __stdout));;
   }
   putc('\r', (& __stdout));;
   putc('\n', (& __stdout));;
   DisplayCmd( "?", "", "List menu commands" );
   DisplayCmd( "help", "", "List menu commands" );
   DisplayCmd( "quit", "", "Exit this menu" );
